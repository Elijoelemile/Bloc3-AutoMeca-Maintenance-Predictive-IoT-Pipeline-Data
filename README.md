...
├── tests/
│   ├── unit/                  # 52 tests, simules — aucune connexion reelle
...
├── docker/                    # deploiement Kafka + ClickHouse + Grafana (instance Compute)
│   ├── clickhouse-init/         # DDL executes automatiquement au demarrage du conteneur
│   │   ├── 01_telemetrie.sql          # copie figee du DDL Bloc 2
│   │   ├── 02_ingestion_timestamp.sql # copie de dashboard/00_add_ingestion_timestamp.sql
│   │   ├── 03_quarantine.sql          # copie de quality/00_add_quarantine_table.sql
│   │   └── 04_materialized_view_telemetrie.sql
│   └── grafana-provisioning/    # datasource ClickHouse + dashboard + alerting, auto-charges
├── docker-compose.yml         # Kafka (KRaft) + ClickHouse + Grafana — deploiement reel (voir README)
├── .env.example
├── .gitignore
├── requirements.txt
└── README.md
...
- **`docker/` + `docker-compose.yml`** — déploiement réel de Kafka (image officielle `apache/kafka`, mode KRaft — pas de Zookeeper), ClickHouse (DDL initialisé automatiquement depuis `docker/clickhouse-init/`, 4 scripts numérotés dans l'ordre : télémétrie du Bloc 2, horodatage d'ingestion, quarantaine, vue matérialisée) et Grafana (datasource + dashboard + règles d'alerte provisionnés depuis `docker/grafana-provisioning/`, adapté du provisioning vérifié manuellement — voir `dashboard/` ci-dessus). Déployé avec succès sur une instance Compute Scaleway, aux côtés des conteneurs du Bloc 4 (voir README du Bloc 2, section "Contraintes de coût et choix d'infrastructure") : les 3 services démarrés, ClickHouse peuplé par les 4 scripts, datasource Grafana connectée réellement (`"status":"OK"`), dashboard chargé.
- **`tests/unit/`** — 52 tests couvrant l'extraction, le chargement staging, le pipeline temps réel, la consolidation, la qualité des données, la pseudonymisation et l'orchestration, sans connexion réelle (simulés)
